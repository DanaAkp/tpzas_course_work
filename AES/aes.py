...
class AES:
    ...
    def encrypt_block(self, plaintext):
        ...
        self.add_round_key(plain_state, self._key_matrix[0])

        for i in range(1, self.n_rounds):
            self.sub_bytes(plain_state)
            self.shift_rows(plain_state)
            self.mix_columns(plain_state)
            self.add_round_key(plain_state, self._key_matrix[i])

        self.sub_bytes(plain_state)
        self.shift_rows(plain_state)
        self.add_round_key(plain_state, self._key_matrix[-1])
    ...
    def decrypt_block(self, ciphertext):
        ...
        self.add_round_key(cipher_state, self._key_matrix[-1])
        self.inv_shift_rows(cipher_state)
        self.inv_sub_bytes(cipher_state)

        for i in range(self.n_rounds - 1, 0, -1):
            self.add_round_key(cipher_state, self._key_matrix[i])
            self.inv_mix_columns(cipher_state)
            self.inv_shift_rows(cipher_state)
            self.inv_sub_bytes(cipher_state)

        self.add_round_key(cipher_state, self._key_matrix[0])
